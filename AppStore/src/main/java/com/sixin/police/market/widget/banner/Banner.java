package com.sixin.police.market.widget.banner;
...
import com.sixin.police.market.widget.banner.transformers.BasePageTransformer;
...
public class Banner extends RelativeLayout implements BannerViewPager.AutoPlayDelegate, ViewPager.OnPageChangeListener {
    ...
    /** 默认指示点资源 */
    private int mPointDrawableResId = R.drawable.banner_selector_point;
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null) {
            mIsAutoPlay = typedArray.getBoolean(R.styleable.Banner_isAutoPlay, true);
            mIsHandLoop = typedArray.getBoolean(R.styleable.Banner_isHandLoop, false);
            mIsTipsMarquee = typedArray.getBoolean(R.styleable.Banner_isTipsMarquee, false);
            mAutoPalyTime = typedArray.getInteger(R.styleable.Banner_AutoPlayTime, 5000);
            mPointsIsVisible = typedArray.getBoolean(R.styleable.Banner_pointsVisibility, true);
            mPointPosition = typedArray.getInt(R.styleable.Banner_pointsPosition, CENTER);
            mPointContainerLeftRightPadding = typedArray.getDimensionPixelSize(R.styleable.Banner_pointContainerLeftRightPadding, mPointContainerLeftRightPadding);
            mPointLeftRightPading = typedArray.getDimensionPixelSize(R.styleable.Banner_pointLeftRightPadding, mPointLeftRightPading);
            mPointTopBottomPading = typedArray.getDimensionPixelSize(R.styleable.Banner_pointTopBottomPadding, mPointTopBottomPading);
            mPointContainerPosition = typedArray.getInt(R.styleable.Banner_pointContainerPosition, BOTTOM);
            mPointContainerBackgroundDrawable = typedArray.getDrawable(R.styleable.Banner_pointsContainerBackground);
            mPointNoraml = typedArray.getDrawable(R.styleable.Banner_pointNormal);
            mPointSelected = typedArray.getDrawable(R.styleable.Banner_pointSelect);
            mPointDrawableResId = typedArray.getResourceId(R.styleable.Banner_indicatorDrawable, mPointDrawableResId);
            mTipTextColor = typedArray.getColor(R.styleable.Banner_tipTextColor, mTipTextColor);
            mTipTextSize = typedArray.getDimensionPixelSize(R.styleable.Banner_tipTextSize, mTipTextSize);
            mIsNumberIndicator = typedArray.getBoolean(R.styleable.Banner_isShowNumberIndicator, mIsNumberIndicator);
            mNumberIndicatorBackground = typedArray.getDrawable(R.styleable.Banner_numberIndicatorBacgroud);
            mIsShowIndicatorOnlyOne = typedArray.getBoolean(R.styleable.Banner_isShowIndicatorOnlyOne, mIsShowIndicatorOnlyOne);
            mPageChangeDuration = typedArray.getInt(R.styleable.Banner_pageChangeDuration, mPageChangeDuration);
            mPlaceholderDrawableResId = typedArray.getResourceId(R.styleable.Banner_placeholderDrawable, mPlaceholderDrawableResId);
            mIsClipChildrenMode = typedArray.getBoolean(R.styleable.Banner_isClipChildrenMode, false);
            mClipChildrenLeftRightMargin = typedArray.getDimensionPixelSize(R.styleable.Banner_clipChildrenLeftRightMargin, mClipChildrenLeftRightMargin);
            mClipChildrenTopBottomMargin = typedArray.getDimensionPixelSize(R.styleable.Banner_clipChildrenTopBottomMargin, mClipChildrenTopBottomMargin);
            mViewPagerMargin = typedArray.getDimensionPixelSize(R.styleable.Banner_viewpagerMargin, mViewPagerMargin);
            mIsClipChildrenModeLessThree = typedArray.getBoolean(R.styleable.Banner_isClipChildrenModeLessThree, false);
            typedArray.recycle();
        }
            ...
            if (mOnItemClickListener != null && !mDatas.isEmpty()) {
                view.setOnClickListener(new OnDoubleClickListener() {
                    ...
                    public void onNoDoubleClick(View v) {
                        mOnItemClickListener.onItemClick(Banner.this, mDatas.get(realPosition), v, realPosition);
                    }
                });
            }

            if (null != mAdapter && !mDatas.isEmpty()) {
                mAdapter.loadBanner(Banner.this, mDatas.get(realPosition), view, realPosition);
            }
            ...
            if (getRealCount() > 0 && (mIsShowIndicatorOnlyOne || !mIsOneImg)) {
                ...
                for (int i = 0; i < getRealCount(); i++) {
                    ...
                    if (mPointNoraml != null && mPointSelected != null) {
                        imageView.setImageDrawable(BannerUtils.getSelector(mPointNoraml, mPointSelected));
                    } else {
    ...
    private static class AutoSwitchTask implements Runnable {
        private final WeakReference<Banner> mXBanner;

        private AutoSwitchTask(Banner mXBanner) {
            this.mXBanner = new WeakReference<>(mXBanner);
        }

        @Override
        public void run() {
            Banner banner = mXBanner.get();
            if (banner != null) {
    ...
    public interface OnItemClickListener {
        void onItemClick(Banner banner, Object model, View view, int position);
    }

    public interface XBannerAdapter {
        void loadBanner(Banner banner, Object model, View view, int position);
    }
}
